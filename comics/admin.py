...
@admin.action(description='Delete a series chapters and images')
def delete_series(modeladmin, request, queryset):
    for q in queryset:
        if q.chapters.all():
            ...
            messages.info(request, "Chapters and images successfully deleted.")
        else:
            messages.info(request, "No chapters to delete.")
